...
import { CALLER_ADDRESS, SelectorType, THIS_ADDRESS } from "@/test/utils/contants";
...
describe("TraderJoeRouter", () => {
  ...
  describe("#swapTokensForExactAVAX", () => {
    ...
    it("should swap tokens for exact AVAX with rest (arbitrary receiver)", async () => {
      const tx = masterProxy
        .connect(CALLER)
        .make([
  ...
  describe("#swapAVAXForExactTokens", () => {
    ...
    it("should swap AVAX for exact tokens with rest (arbitrary receiver)", async () => {
      const tx = masterProxy
        .connect(CALLER)
        .make(
          [
